...
func (r *ProxyRequestRepository) Create(p *domain.ProxyRequest) error {
	...
	result, err := r.db.db.Exec(
		`INSERT INTO proxy_requests (created_at, updated_at, request_id, session_id, client_type, request_model, response_model, start_time, end_time, duration_ms, status, request_info, response_info, error, proxy_upstream_attempt_count, final_proxy_upstream_attempt_id, input_token_count, output_token_count, cache_read_count, cache_write_count, cost) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.CreatedAt, p.UpdatedAt, p.RequestID, p.SessionID, p.ClientType, p.RequestModel, p.ResponseModel,
...
func (r *ProxyRequestRepository) Update(p *domain.ProxyRequest) error {
	...
	_, err := r.db.db.Exec(
		`UPDATE proxy_requests SET updated_at = ?, request_id = ?, session_id = ?, client_type = ?, request_model = ?, response_model = ?, start_time = ?, end_time = ?, duration_ms = ?, status = ?, request_info = ?, response_info = ?, error = ?, proxy_upstream_attempt_count = ?, final_proxy_upstream_attempt_id = ?, input_token_count = ?, output_token_count = ?, cache_read_count = ?, cache_write_count = ?, cost = ? WHERE id = ?`,
		p.UpdatedAt, p.RequestID, p.SessionID, p.ClientType, p.RequestModel, p.ResponseModel,
		nullTime(p.StartTime), nullTime(p.EndTime), p.Duration.Milliseconds(), p.Status,
		toJSON(p.RequestInfo), toJSON(p.ResponseInfo), p.Error,
		p.ProxyUpstreamAttemptCount, p.FinalProxyUpstreamAttemptID,
...
func (r *ProxyRequestRepository) GetByID(id uint64) (*domain.ProxyRequest, error) {
	row := r.db.db.QueryRow(`SELECT id, created_at, updated_at, request_id, session_id, client_type, request_model, response_model, start_time, end_time, duration_ms, status, request_info, response_info, error, proxy_upstream_attempt_count, final_proxy_upstream_attempt_id, input_token_count, output_token_count, cache_read_count, cache_write_count, cost FROM proxy_requests WHERE id = ?`, id)
	return r.scanRequest(row)
}

func (r *ProxyRequestRepository) List(limit, offset int) ([]*domain.ProxyRequest, error) {
	rows, err := r.db.db.Query(`SELECT id, created_at, updated_at, request_id, session_id, client_type, request_model, response_model, start_time, end_time, duration_ms, status, request_info, response_info, error, proxy_upstream_attempt_count, final_proxy_upstream_attempt_id, input_token_count, output_token_count, cache_read_count, cache_write_count, cost FROM proxy_requests ORDER BY id DESC LIMIT ? OFFSET ?`, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *ProxyRequestRepository) scanRequestRows(rows *sql.Rows) (*domain.ProxyRequest, error) {
	...
	var durationMs int64
	var reqInfoJSON, respInfoJSON string
	err := rows.Scan(&p.ID, &p.CreatedAt, &p.UpdatedAt, &p.RequestID, &p.SessionID, &p.ClientType, &p.RequestModel, &p.ResponseModel, &startTime, &endTime, &durationMs, &p.Status, &reqInfoJSON, &respInfoJSON, &p.Error, &p.ProxyUpstreamAttemptCount, &p.FinalProxyUpstreamAttemptID, &p.InputTokenCount, &p.OutputTokenCount, &p.CacheReadCount, &p.CacheWriteCount, &p.Cost)
	if err != nil {
		return nil, err
	}
	p.StartTime = parseTime(startTime)
